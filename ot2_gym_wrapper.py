...
class OT2Env(gym.Env):
    ...
    def step(self, action):
        ...
        reward = ((self.prev_d_goal / d_goal_max) - (d_goal / d_goal_max))*1500
        if reward > 0:
            reward += 25 * ((d_goal_max - d_goal) / d_goal_max)
        else:
        ...
        if d_goal < 0.001:
            ...
            if d_goal < 0.001 and observation[6] < 0.02:
                terminated = True
                self.steps_taken_2_stop = self.steps - self.step_2_stop
                self.reward_at_stop =  max(25, 80 - 5 * self.steps_taken_2_stop)
